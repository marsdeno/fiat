...
SUBROUTINE MPL_ALLREDUCE_INT(KSENDBUF,CDOPER,LDREPROD, &
                            & KCOMM,KERROR,CDSTRING,LDDEVICE_DATA)
...
IF ( MPL_NUMPROC > 1 ) THEN
  !$ACC HOST_DATA USE_DEVICE(KSENDBUF) IF(LL_ON_DEVICE)
...
LOGICAL            :: LLREPRODSUM, LL_ON_DEVICE
INTEGER(KIND=JPIM) :: ISENDCOUNT,IERROR
TYPE(MPI_COMM)     :: ICOMM
TYPE(MPI_OP)       :: IOPER
...
ELSE  
  IF ( MPL_NUMPROC > 1 ) THEN
    !$ACC HOST_DATA USE_DEVICE(PSENDBUF) IF(LL_ON_DEVICE)
