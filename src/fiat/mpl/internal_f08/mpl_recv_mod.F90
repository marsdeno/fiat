...
SUBROUTINE MPL_RECV_REAL4(PBUF,KSOURCE,KTAG,KCOMM,KFROM,KRECVTAG,&
 &KOUNT,KMP_TYPE,KERROR,KREQUEST,CDSTRING,LDDEVICE_DATA)
...
SUBROUTINE MPL_RECV_INT(KBUF,KSOURCE,KTAG,KCOMM,KFROM,KRECVTAG,KOUNT,&
 &KMP_TYPE,KERROR,KREQUEST,CDSTRING,LDDEVICE_DATA)
...
!$ACC HOST_DATA USE_DEVICE(PREAL4) IF(LL_ON_DEVICE)
IF(IMP_TYPE == JP_BLOCKING_STANDARD) THEN
...
!$ACC HOST_DATA USE_DEVICE(PREAL8) IF(LL_ON_DEVICE)
IF(IMP_TYPE == JP_BLOCKING_STANDARD) THEN
...
!IF(LL_ON_DEVICE) THEN
!  !$ACC HOST_DATA USE_DEVICE(CDCHAR)
!ENDIF
IF(IMP_TYPE == JP_BLOCKING_STANDARD) THEN
